...
def lind_fs_openat(args):
    ...
    try:
        result_fd = openat_syscall(dir_fd, file_name, flags, mode)
    except SyscallError, e:
        message = "Could not find file: " + file_name + ":\n" + str(e[2]) + "\n"
        return ErrorResponseBuilder("fs_open", e[1], message)
    return SuccessResponseBuilder("fs_open", result_fd)
...
def lind_safe_fs_openat(args):
    """ Safely wrap the openat call.

    See dispatcher.repy for details.

    Check the mode flags and file for consistancy,
    then call the real openat dispatcher.
    ...
    assert isinstance(dir_fd, int), "the dir_fd should be an int"
    assert isinstance(flags, int), "the flags should be an int"
    assert isinstance(mode, int), "the mode should be an int"
    assert isinstance(file_name, str), "filename should be a string"
    assert ((flags & MASK) == O_RDONLY or
...
#begin include net_sendmsg.repy
...
#begin include net_recvmsg.repy
...
def lind_net_getpeername(args):
  ...
  try:
    ip, port = getpeername_syscall(sockfd)
  except SyscallError,e:
...
def lind_net_getsockname(args):
  ...
  try:
    ip, port = getsockname_syscall(sockfd)
  except SyscallError,e:
...
def lind_net_shutdown(args):
    ...
    how = args[1]
    try:
        result = netshutdown_syscall(sockfd, how)
...
def unimplemented(who):
  ...
  # so we should exit.
  message = "an unimplemented function has been called " + str(who)
  if PRODUCTION:
    log("error: " + message)
    exitall()
  else:
...
# Yiwen: this looks really confusing
...
def get_lind_map_to_native(lind):
  ...
  if len(FILE_MAP) < 4:     # A bug here: suggested by Justin    
    return lind
